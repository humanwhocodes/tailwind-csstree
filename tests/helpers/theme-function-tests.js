	...
	describe("theme() function", () => {
		describe("Parsing", () => {
			it("should parse theme() with colors.green.500", () => {
				const tree = toPlainObject(
					parse("a { color: theme(colors.green.500); }"),
				);
			...
			it("should parse theme() with colors.red.500 / 50%", () => {
				const tree = toPlainObject(
					parse("a { color: theme(colors.red.500 / 50%); }"),
				);
				assert.deepStrictEqual(tree, {
					type: "StyleSheet",
					loc: null,
			...
			it("should parse theme() with colors.gray.900/75%", () => {
				const tree = toPlainObject(
					parse("a { color: theme(colors.gray.900/75%); }"),
				);
				assert.deepStrictEqual(tree, {
					type: "StyleSheet",
					loc: null,
			...
			it("should parse theme() with spacing[2.5]", () => {
				const tree = toPlainObject(
					parse("a { margin: theme(spacing[2.5]); }"),
				);
				assert.deepStrictEqual(tree, {
					type: "StyleSheet",
					loc: null,
		...
		describe("Validation", () => {
			describe("Type Validation", () => {
				["theme(spacing.12)", "theme(spacing.4)"].forEach(value => {
					it(`should validate type <tw-theme-spacing> with ${value}`, () => {
						const tree = parse(value, { context: "value" });
						assert.strictEqual(
							lexer.matchType("tw-theme-spacing", tree).error,
							null,
						);
					});
				});
				...
				[
					...
					it(`should validate type <tw-theme-color> with ${value}`, () => {
						const tree = parse(value, { context: "value" });
						assert.strictEqual(
							lexer.matchType("tw-theme-color", tree).error,
							null,
						);
					});
				});
			...
			describe("Property Validation", () => {
				it("should validate margin: theme(spacing.4)", () => {
					const tree = toPlainObject(
						parse("a { margin: theme(spacing.4); }"),
					);
